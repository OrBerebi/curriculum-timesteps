...
def timesteps_to_sigma(timesteps, alphas_cumprod, loss=None, epsilon=1e-6):
    """
    Convert timesteps to sigmas, optionally modulating them based on loss values.

    Args:
        timesteps (torch.Tensor): The timesteps for which to calculate sigmas.
        alphas_cumprod (torch.Tensor): Cumulative alpha values for timesteps.
        loss (torch.Tensor, optional): Scalar loss value used to modulate sigma. If None, no modulation is applied.
        epsilon (float, optional): Small constant to prevent zero scaling.

    Returns:
        torch.Tensor: Adjusted sigmas.
    """
    ...
    if loss is not None:
        # Compute noise gain from loss
        loss_sigmoid = torch.sigmoid(loss.detach())  # Detach to avoid backprop issues
        noise_gain = 1 - loss_sigmoid + epsilon

        # Modulate sigma with noise gain
        sqrt_one_minus_alpha_prod = sqrt_one_minus_alpha_prod * noise_gain
    else:
        noise_gain = 1

    return sqrt_one_minus_alpha_prod, noise_gain
...
def run_unet(pipe, noisy_latent, steps, resolution=None, captions=None, return_noise=False):
    ...
    prompt_embeds = encoded_prompt["prompt_embeds"].to(device)
    pooled_prompt_embeds = encoded_prompt["pooled_prompt_embeds"]

    add_time_ids = torch.cat([compute_time_ids((resolution, resolution), (0, 0), device = device) for _ in range(noisy_latent.shape[0])])
    unet_added_conditions = {"time_ids": add_time_ids.to(device), "text_embeds": pooled_prompt_embeds.to(device)}
    noise_pred = pipe.unet(noisy_latent, steps, prompt_embeds, added_cond_kwargs=unet_added_conditions).sample
    if return_noise:
        return noise_pred
    
    sigma, noise_gain = timesteps_to_sigma(steps, pipe.scheduler.alphas_cumprod.to(steps.device))
...
def sample_with_early_stop(pipe, denoising_end, prompts, num_inference_steps=50, return_latent=False, **pipe_kwargs):
    ...
    if return_latent:
        ...
        images = pipe.vae.decode(clean.to(pipe.vae.dtype) / pipe.vae.config.scaling_factor).sample
